...
def evaluate_model(model_name="best", episodes=1):
    ...
    # Create environment — NO randomization during eval
...
def _compute_metrics(results_df, trades_df, env):
    """Compute comprehensive evaluation metrics using equity curve."""
    if trades_df.empty:
        return {
            "model": "PPO",
    ...
    # Equity-based metrics (from env's equity curve)
    equity_metrics = _compute_equity_metrics(env)
...
def _compute_equity_metrics(env):
    """Compute metrics from the equity curve tracked in the environment."""
    curve = np.array(env.equity_curve) if hasattr(env, 'equity_curve') and env.equity_curve else None

    if curve is None or len(curve) < 2:
        return {"max_drawdown_pct": 0.0, "sharpe_ratio": 0.0, "sortino_ratio": 0.0}

    # Max drawdown from equity curve (percentage)
    peak = np.maximum.accumulate(curve)
    dd = (peak - curve) / np.where(peak > 0, peak, 1)
    max_dd_pct = round(float(dd.max()) * 100, 2)

    # Daily equity returns (resample M1 equity to daily by taking last value per day)
    # Approximate: 1 trading day ≈ 1440 M1 bars, use 1000 as practical estimate
    daily_interval = min(1000, max(100, len(curve) // 20))
    daily_equity = curve[::daily_interval]
    if len(daily_equity) < 2:
        daily_equity = curve[::max(1, len(curve) // 10)]
    ...
    if len(daily_returns) > 1 and daily_returns.std() > 0:
        sharpe = round(float((daily_returns.mean() / daily_returns.std()) * np.sqrt(252)), 4)
    else:
        sharpe = 0.0
    ...
    if len(downside) > 0 and downside.std() > 0:
        sortino = round(float((daily_returns.mean() / downside.std()) * np.sqrt(252)), 4)
    else:
        sortino = 0.0
...
def _compute_drawdown(trades_df):
    """Max drawdown from trade P/L series (fallback if no equity curve)."""
...
def _compute_sharpe(trades_df, risk_free=0):
    """Annualized Sharpe ratio from trade P/Ls (fallback)."""
    if trades_df.empty or "pnl" not in trades_df.columns:
        return 0.0
    pnls = trades_df["pnl"]
    if pnls.std() == 0:
