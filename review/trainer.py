...
def menu_train():
    ...
    print(f"  [M] Multi-month (parallel)")
    ...
    if mode == "S":
        ...
        start = int(input(f"  Start month index [0-{len(train_chunks)-1}]: ").strip())
    ...
    iterations = int(input("  Iterations (default 3): ").strip() or "3")
    steps_per = int(input("  Steps per iteration (default 20000): ").strip() or "20000")

    # Create env(s) — WITH randomization for training
    if len(selected) == 1:
